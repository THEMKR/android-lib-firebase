...
abstract class BaseFirebaseTask<MKR> {
    ...
    private fun doInBackground(): FirebaseResponse<MKR> {
        ...
        if (firebaseData != null) {
            if (firebaseData.dataArray.size == 1) {
                return if (firebaseData.dataArray.get(0) is DataSnapshot) {
                    FirebaseResponse(parseFirebaseDataSnapShot(firebaseData.dataArray.get(0) as DataSnapshot))
                } else if (firebaseData.dataArray.get(0) is DatabaseError) {
                    parseFirebaseDataError(firebaseData.dataArray.get(0) as DatabaseError)
                } else {
                    getMiscellaneousError()
                }
            } else {
                return FirebaseResponse(parseFirebaseDataSnapShot(firebaseData.dataArray))
            }
        } else {
